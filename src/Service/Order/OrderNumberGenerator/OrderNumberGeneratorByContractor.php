...
class OrderNumberGeneratorByContractor implements OrderNumberGeneratorInterface
{
    ...
    public function generate(Order $order): OrderNumber
    {
        if ($order->getContractor()) {
            $lastOrder = $this->orderRepository->findLastNumberedOrderBySupplier($order->getSupplierCompany());
